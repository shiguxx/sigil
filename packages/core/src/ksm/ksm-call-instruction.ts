...
type SigilKSMCallArgument = SigilKSMVariable | SigilKSMExpression;
...
class SigilKSMCallInstruction extends SigilKSMInstruction {
  ...
  protected _build(buffer: CTRMemory, ctx: SigilKSMContext): void {
    ...
    for (const argument of this.arguments) {
      if (argument instanceof SigilKSMVariable) {
        buffer.u32(argument.id);
        continue;
      }

      ctx.expr(buffer, argument);
    }
  ...
  protected _parse(buffer: CTRMemory, ctx: SigilKSMContext): void {
    ...
    while (id !== SigilKSMOpCode.OPCODE_CALL_END) {
      if (isConst) {
        ...
        this.arguments.push(ctx.expr(buffer, ctx.intr(id) || ctx.symb(id)));
      }
  ...
  protected override _sizeof(): number {
    ...
    for (const argument of this.arguments) {
      sizeof += Array.isArray(argument) ? exprsizeof(argument) : argument.sizeof;
    }
