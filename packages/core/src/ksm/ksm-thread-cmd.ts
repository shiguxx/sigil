...
class SigilKSMThreadInstruction extends SigilKSMInstruction {
  public take: number[];
  ...
  public override get opcode(): number {
    return SigilKSMOpCode.OPCODE_THREAD;
  }
  ...
  protected _parse(buffer: CTRMemory, ctx: SigilKSMContext): void {
    ...
    if (this.callee instanceof SigilKSMFunction) {
      const thisInstructionSize =
        CTRMemory.U32_SIZE + // thread opcode
...
export {
  ...
  SigilKSMThreadInstruction as KSMThreadInstruction
};
